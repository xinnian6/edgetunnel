async function fetchUUIDFromLink(uuidLink) {
    try {
        ...
        if (uuid.includes('<') && uuid.includes('>')) {
            ...
            tempDiv.innerHTML = uuid;
            uuid = tempDiv.textContent || tempDiv.innerText || '';
            uuid = uuid.trim();
        }
...
// 如果 userID 是一个链接，尝试获取内容
if (userID.startsWith('http://') || userID.startsWith('https://')) {
    const fetchedContent = await fetchUUIDFromLink(userID);
    if (fetchedContent) {
        userID = fetchedContent;
    }
}
